...
class CalendarStrip extends Component {
  ...
  static defaultProps = {
    ...
    showDate: true,
    updateWeek: true,
    iconLeft: require('../../assets/images/left-arrow-black.png'),
    iconRight: require('../../assets/images/right-arrow-black.png'),
  ...
  constructor(props) {
    ...
    const startingDate = this.getInitialStartingDate(selectedDate);
  ...
  getInitialStartingDate(selectedDate) {
    const me = this;
    console.log(
      'getInitialStartingDate',
      me.setLocale(selectedDate.clone().add(-3, 'days')),
    );
    //this.setState({ startingDate: me.setLocale(moment(this.props.selectedDate).add(-3, "days")) })
    // if (this.props.startingDate) {
    //   return this.setLocale(moment(this.props.startingDate));
    //   console.log('startingDate: ' + this.setLocale(moment(this.props.startingDate)))
    // } else {
    //   console.log('isoWeekday: ' + this.props.selectedDate)
    //   return this.setLocale(moment(this.props.selectedDate)).isoWeekday(1);

    // }
    return me.setLocale(selectedDate.clone().add(-3, 'days'));
  }

  //Set startingDate to the previous week
  getPreviousWeek() {
    console.log('getPreviousWeek', this.state.startingDate);
    ...
    //   }
    // }
    let weekData = this.updateWeekData1(0, previousWeekStartDate);
    this.setState({startingDate: previousWeekStartDate, ...weekData});
  }

  //Set startingDate to the next week
  getNextWeek() {
    const nextWeekStartDate = this.state.startingDate.clone().add(7, 'days');
    if (this.props.onWeekChanged) {
      if (this.props.useIsoWeekday) {
    ...
    let weekData = this.updateWeekData1(1, nextWeekStartDate);
  ...
  updateWeekData1(
    ...
    for (let i = 0; i < this.numDaysInWeek; i++) {
      ...
      console.log(i + ' = ios date', date);

      // if (props.useIsoWeekday) {
      //   console.log('props.useIsoWeekday', props.useIsoWeekday)
      //   // isoWeekday starts from Monday
      //   date = me.setLocale(selectedDate.clone().isoWeekday(i + 1));
  ...
  updateWeekData(
    ...
    console.log('updateWeekData startingDate', startingDate);
    startingDate = me.setLocale(selectedDate.clone().add(-3, 'days'));
    this.setState({
      startingDate: me.setLocale(selectedDate.clone().add(-3, 'days')),
    });

    //console.log('updateWeekData startingDate', this.state.startingDate)
  ...
  onDateSelected(selectedDate) {
    this.setState({
      selectedDate,
      ...this.updateWeekData2(this.state.startingDate, selectedDate),
    });
    ...
    for (let i = 0; i < datesForWeek.length; i++) {
      ...
      let calendarDay = (
        <_CalendarDay
          ...
          showMonthName={this.props.showMonthName}
